class ImportNames
  ...
  def wrapped_ens_collection
    @wrapped_ens_collection ||= Collection.find_or_create_by!(name: 'Wrapped ENS', contract_address: Ethereum::Chain::ENS_WRAPPER_ADDRESS)
  end

  def ens_collection
    @ens_collection ||= Collection.find_or_create_by!(name: 'ENS', contract_address: Ethereum::Chain::ENS_CONTRACT_ADDRESS)
  end

  def wrapped?(data)
    data['registrant']['id'].downcase ==  Ethereum::Chain::ENS_WRAPPER_ADDRESS.downcase
  end
